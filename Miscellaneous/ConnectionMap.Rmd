...
# 
# full_data <- file.choose()

ggplot() +
geom_path(data = shapefile_df,
          aes(x = along, y = alat, group = group),
...
p.local <- ggplot(animation_local, aes(rank, group = Virus_Name,
  ...
  geom_text(aes(y = 0, label = paste(Virus_Name, " ")), vjust = 0.2, hjust = 1, size = 11) +
  geom_text(aes(y = round(IR,2), label = round(IR_lbl,2), hjust = 0), size = 11) +
