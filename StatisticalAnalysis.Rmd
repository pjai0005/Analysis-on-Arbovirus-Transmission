...
nb_data <- long_data %>% 
  filter(str_detect(Type, "IR")) %>% 
  dplyr::select(Humanpop, donationrate, Value) %>% 
  rename(Donation  = donationrate,
         Population = Humanpop) %>% 
  mutate(Donation = as.integer(Donation),
         Population = as.integer(Population),
         Value = as.integer(Value))


paste("Mean of IR", round(mean(nb_data$Value), 4))
paste("Variance of IR", round(var(nb_data$Value), 4))

paste("Variance Ratio", round(var(nb_data$Value) / mean(nb_data$Value), 4)) 
...
# Summary stats
```{r}
negb1 <- glm.nb(Value ~ Donation , data = nb_data)

negb <- glm.nb(Value ~ Donation+Population , data = nb_data)

pois <- glm(Value ~ Donation+Population , "poisson", data = nb_data) 


summary(negb1)
summary(pois)
summary(negb) # has lower AIC therefore better model
...
Degree of freedom: However, for a well-fitting model, the residual deviance should be close to the degrees of freedom (74), which is not the case here. For example, this could be a result of over dispersion where the variation is greater than predicted by the model. 

# Zero Inflated Count Data Regression

```{r}

ggplot(nb_data, aes(x = Value))+
  geom_histogram(bins = 30, color = "dark blue", fill = "steel blue")+ 
    scale_x_continuous(breaks = seq(0, 35, by = 5))+
    scale_y_break(c(490, 1e+05), scales = 0.5)+
  theme_bw()+
  labs(title = "Histogram of the data", x = "Incidence Rate",
       subtitle = "to check for Zero Inflated Count Regression")

zinb <- zeroinfl(nb_data$Value ~ nb_data$Donation, link = "logit",
...
AIC(negb, zinb, negb1, pois)


paste("Dispersion Parameter", negb$theta) 


paste("Null Deviance", negb$null.deviance) 


paste("Residual Deviance", negb$deviance) 


paste("Degree of Freedom", negb$df.residual) 


countreg::rootogram(negb)
```
